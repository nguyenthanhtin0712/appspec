...
const SpecialtyDialog = () => {
  ...
  return (
    <Dialog open={specialtyDialog.open} onClose={handleClose} maxWidth="xs">
      <DialogTitle>{specialtyDialog.action === 'add' ? 'Thêm sinh viên' : 'Chỉnh sửa sinh viên'}</DialogTitle>
      ...
      <Formik
        ...
        validationSchema={Yup.object().shape({
          specialty_code: Yup.string().max(255).required('Mã sinh viên là bắt buộc !'),
          specialty_name: Yup.string().max(255).required('Tên sinh viên là bắt buộc !'),
          major_id: Yup.string().max(255).required('Vui lòng chọn sinh viên !')
        })}
          ...
          <form noValidate onSubmit={handleSubmit}>
            ...
            <DialogActions>
              ...
              <Button variant="contained" type="submit" disabled={isSubmitting}>
                {specialtyDialog.action === 'add' ? 'Thêm sinh viên' : 'Chỉnh sửa'}
              </Button>
            </DialogActions>
          </form>
        )}
